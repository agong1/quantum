...
class LocalVLANMapping:
    ...
    def __str__(self):
        return (_("lv-id = %(lv-id)s type = %(type)s "
                  "phys-net = %(phys-net)s phys-id = %(phys-id)s") %
                {'lv-id': self.vlan, 'type': self.network_type,
                 'phys-net': self.physical_network,
                 'phys-id': self.segmentation_id})
...
class OFANeutronAgent(sg_rpc.SecurityGroupAgentRpcCallbackMixin):
    ...
    def _check_ovs_version(self):
        if p_const.TYPE_VXLAN in self.tunnel_types:
            try:
                ovs_lib.check_ovs_vxlan_version(
